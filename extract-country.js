// extract-country.js — Generic extractor: pull all units for a country from CMO DB
// Usage: node extract-country.js <code> <countryId>
//   e.g. node extract-country.js ru 2079
//        node extract-country.js su 2088
const Database = require('./node_modules/better-sqlite3');
const fs = require('fs');
const path = require('path');

const code = process.argv[2];
const countryId = parseInt(process.argv[3], 10);
if (!code || !countryId) { console.error('Usage: node extract-country.js <code> <countryId>'); process.exit(1); }
...
console.log(`\nExtracting country "${code}" (ID ${COUNTRY})...\n`);
...
const whTypes = em('EnumWarheadType'), snCaps = em('EnumSensorCapability'), snFreqs = em('EnumSensorFrequency');
...
const allAC = db.prepare(`SELECT * FROM DataAircraft WHERE OperatorCountry=? AND Hypothetical=0 ORDER BY YearCommissioned DESC`).all(COUNTRY);
const acGroups = {};
for (const r of allAC) { if (!acGroups[r.Name]) acGroups[r.Name] = r; }

const acIndex = [], acDetails = {};
let acId = 0;
for (const [, row] of Object.entries(acGroups).sort((a, b) => a[0].localeCompare(b[0]))) {
  acId++;
...
// ========== SHIPS + SUBS ==========
console.log('=== Ships & Submarines ===');
const allShips = db.prepare(`SELECT * FROM DataShip WHERE OperatorCountry=? AND Hypothetical=0 ORDER BY YearCommissioned DESC`).all(COUNTRY);
const allSubs = db.prepare(`SELECT * FROM DataSubmarine WHERE OperatorCountry=? AND Hypothetical=0 ORDER BY YearCommissioned DESC`).all(COUNTRY);
const shipGroups = {};
for (const r of allShips) { if (!shipGroups[r.Name]) shipGroups[r.Name] = { ...r, isSub: false }; }
for (const r of allSubs) { if (!shipGroups[r.Name]) shipGroups[r.Name] = { ...r, isSub: true }; }

const shipIndex = [], shipDetails = {};
let shipId = 0;
for (const [, row] of Object.entries(shipGroups).sort((a, b) => a[0].localeCompare(b[0]))) {
  shipId++;
  const displayName = cleanName(row.Name);
...
for (const r of allAC) {
  ...
  for (const lid of lids) db.prepare('SELECT ComponentID FROM DataLoadoutWeapons WHERE ID=?').all(lid).forEach(x => wpnDbIds.add(x.ComponentID));
}
for (const r of allShips) {
  db.prepare('SELECT ComponentID FROM DataShipMounts WHERE ID=?').all(r.ID).map(x => x.ComponentID)
    .forEach(mid => db.prepare('SELECT ComponentID FROM DataMountWeapons WHERE ID=?').all(mid).forEach(x => wpnDbIds.add(x.ComponentID)));
  db.prepare('SELECT ComponentID FROM DataShipMagazines WHERE ID=?').all(r.ID).map(x => x.ComponentID)
    .forEach(mid => db.prepare('SELECT ComponentID FROM DataMagazineWeapons WHERE ID=?').all(mid).forEach(x => wpnDbIds.add(x.ComponentID)));
}
for (const r of allSubs) {
  db.prepare('SELECT ComponentID FROM DataSubmarineMounts WHERE ID=?').all(r.ID).map(x => x.ComponentID)
    .forEach(mid => db.prepare('SELECT ComponentID FROM DataMountWeapons WHERE ID=?').all(mid).forEach(x => wpnDbIds.add(x.ComponentID)));
}
const allFacs = db.prepare('SELECT * FROM DataFacility WHERE OperatorCountry=? AND Hypothetical=0 ORDER BY YearCommissioned DESC').all(COUNTRY);
for (const r of allFacs) {
  db.prepare('SELECT ComponentID FROM DataFacilityMounts WHERE ID=?').all(r.ID).map(x => x.ComponentID)
    .forEach(mid => db.prepare('SELECT ComponentID FROM DataMountWeapons WHERE ID=?').all(mid).forEach(x => wpnDbIds.add(x.ComponentID)));
}
const wpns = {};
for (const wid of wpnDbIds) {
  const w = db.prepare('SELECT * FROM DataWeapon WHERE ID=?').get(wid);
  if (w && !wpns[w.Name]) wpns[w.Name] = w;
}
const wpnIndex = [], wpnDetails = {};
let wpnId = 0;
for (const [, row] of Object.entries(wpns).sort((a, b) => a[0].localeCompare(b[0]))) {
  wpnId++;
  const displayName = cleanName(row.Name);
...
// ========== SENSORS ==========
console.log('=== Sensors ===');
const snDbIds = new Set();
for (const r of allAC) db.prepare('SELECT ComponentID FROM DataAircraftSensors WHERE ID=?').all(r.ID).forEach(x => snDbIds.add(x.ComponentID));
for (const r of allShips) db.prepare('SELECT ComponentID FROM DataShipSensors WHERE ID=?').all(r.ID).forEach(x => snDbIds.add(x.ComponentID));
for (const r of allSubs) db.prepare('SELECT ComponentID FROM DataSubmarineSensors WHERE ID=?').all(r.ID).forEach(x => snDbIds.add(x.ComponentID));
for (const r of allFacs) db.prepare('SELECT ComponentID FROM DataFacilitySensors WHERE ID=?').all(r.ID).forEach(x => snDbIds.add(x.ComponentID));

const snMap = {};
for (const sid of snDbIds) { const s = db.prepare('SELECT * FROM DataSensor WHERE ID=?').get(sid); if (s && !snMap[s.Name]) snMap[s.Name] = s; }
const snIndex = [], snDetails = {};
let snId = 0;
for (const [, row] of Object.entries(snMap).sort((a, b) => a[0].localeCompare(b[0]))) {
  snId++;
  const displayName = cleanName(row.Name);
...
// ========== FACILITIES (+ split into tabs) ==========
console.log('=== Facilities ===');
const facGroups = {};
for (const r of allFacs) { const key = r.Name.replace(/\s*\[.*?\]\s*$/, '').trim(); if (!facGroups[key]) facGroups[key] = r; }

const facIndex = [], facDetails = {};
let facId = 0;
const infantry = [], armor = [], artillery = [], airdefense = [], radar = [], facilities = [];

for (const [, row] of Object.entries(facGroups).sort((a, b) => a[0].localeCompare(b[0]))) {
  facId++;
  const displayName = cleanName(row.Name);
  ...
  facIndex.push(entry);
  facDetails[facId] = { area: row.Area, mastHeight: row.MastHeight, armorGeneral: row.ArmorGeneral, sensors: uniqueSensors, mounts, signatures: sigs };

  const catDesc = facCats[row.Category] || '';
  const nm = displayName;
  if (catDesc === 'Mobile Personnel') infantry.push(entry);
  else if (catDesc === 'Mobile Vehicle(s)') {
    if (nm.startsWith('Armored') || nm.startsWith('Mech Inf')) armor.push(entry);
    else if (nm.startsWith('Arty') || nm.startsWith('SSM') || nm.startsWith('Support')) artillery.push(entry);
    else if (nm.startsWith('SAM') || nm.startsWith('AAA')) airdefense.push(entry);
    else if (nm.startsWith('Radar') || nm.startsWith('Vehicle') || nm.startsWith('Sensor')) radar.push(entry);
...
// ========== Write output ==========
const outDir = path.join(__dirname, 'data', code);
...
console.log(`\n=== TOTAL: ${acId + shipId + wpnId + snId + facId} units for "${code}" ===\n`);
db.close();
