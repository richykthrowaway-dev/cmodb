// extract-all.js — Auto-discover ALL US units from CMO database
...
// ========== Data extractors (same as extract.js) ==========
...
function cleanName(dbName) {
  // Strip bracket suffixes: "F-14A Tomcat [Early]" -> "F-14A Tomcat"
...
// Group by designation prefix (e.g. "F-14A", "F-14B", "F-14D" are separate)
const acGroups = {};
for (const r of allAC) {
  const m = r.Name.match(/^([A-Z][\w\/-]+\d[A-Z]*)/);
  if (!m) continue;
  const desig = m[1];
  if (!acGroups[desig]) acGroups[desig] = r; // first = latest (ordered DESC)
}
...
for (const r of allSubs) {
  const key = r.Name.replace(/\s*\[.*?\]\s*$/, '').trim();
  if (!subGroups[key]) subGroups[key] = r;
}

const shipIndex = [], shipDetails = {};
let shipId = 0;

// Process surface ships
const sortedShips = Object.entries(shipGroups).sort((a, b) => a[0].localeCompare(b[0]));
for (const [className, row] of sortedShips) {
  shipId++;
  const displayName = cleanName(row.Name);
  console.log(`  ${shipId}: ${displayName} (CMO #${row.ID}, ${row.YearCommissioned || '?'}) [Ship]`);

  const sensors = getShipSensors('DataShipSensors', row.ID);
  const mounts = getMountsWeapons('DataShipMounts', row.ID);
  const sigs = getSignatures('DataShipSignatures', row.ID);
  const codes = getCodes('DataShipCodes', 'EnumShipCode', row.ID);
  const propulsion = getShipPropulsion('DataShipPropulsion', row.ID);
  const magazines = getMagazineWeapons(row.ID, 'DataShipMagazines');
  ...
  shipIndex.push({
    ...
    type: shipTypes[row.Type] || '', operator: services[row.OperatorService] || '',
    commissioned: row.YearCommissioned, crew: row.Crew,
    displacementFull: row.DisplacementFull, length: row.Length, beam: row.Beam, draft: row.Draft,
...
// Process submarines (added to same ships category)
const sortedSubs = Object.entries(subGroups).sort((a, b) => a[0].localeCompare(b[0]));
for (const [className, row] of sortedSubs) {
  shipId++;
  const displayName = cleanName(row.Name);
  console.log(`  ${shipId}: ${displayName} (CMO #${row.ID}, ${row.YearCommissioned || '?'}) [Sub]`);

  const sensors = getShipSensors('DataSubmarineSensors', row.ID);
  const mounts = getMountsWeapons('DataSubmarineMounts', row.ID);
  const sigs = getSignatures('DataSubmarineSignatures', row.ID);
  const codes = getCodes('DataSubmarineCodes', 'EnumSubmarineCode', row.ID);
  const propulsion = getShipPropulsion('DataSubmarinePropulsion', row.ID);
  ...
  shipIndex.push({
    id: shipId, name: displayName, dbName: row.Name, comments: row.Comments || '',
    type: subTypes[row.Type] || '', operator: services[row.OperatorService] || '',
    commissioned: row.YearCommissioned, crew: row.Crew,
    displacementFull: row.DisplacementFull, length: row.Length, beam: row.Beam, draft: row.Draft,
    maxSpeed, sensorCount: uniqueSensors.length, weaponCount: wpnSet.size,
...
// Get weapons used by US platforms, filtered to US-designated weapons
console.log('\n=== Auto-discovering Weapons ===');

const usWpnDbIds = new Set();
// From aircraft loadouts
for (const r of allAC) {
  const lids = db.prepare('SELECT ComponentID FROM DataAircraftLoadouts WHERE ID=?').all(r.ID).map(x => x.ComponentID);
  for (const lid of lids) db.prepare('SELECT ComponentID FROM DataLoadoutWeapons WHERE ID=?').all(lid).forEach(x => usWpnDbIds.add(x.ComponentID));
}
// From ship mounts + magazines
for (const r of allShips) {
  db.prepare('SELECT ComponentID FROM DataShipMounts WHERE ID=?').all(r.ID).map(x => x.ComponentID)
    .forEach(mid => db.prepare('SELECT ComponentID FROM DataMountWeapons WHERE ID=?').all(mid).forEach(x => usWpnDbIds.add(x.ComponentID)));
  db.prepare('SELECT ComponentID FROM DataShipMagazines WHERE ID=?').all(r.ID).map(x => x.ComponentID)
    .forEach(mid => db.prepare('SELECT ComponentID FROM DataMagazineWeapons WHERE ID=?').all(mid).forEach(x => usWpnDbIds.add(x.ComponentID)));
}
// From sub mounts
for (const r of allSubs) {
...
// From facility mounts
const allFacs = db.prepare('SELECT * FROM DataFacility WHERE OperatorCountry=? AND Hypothetical=0 ORDER BY YearCommissioned DESC').all(US);
for (const r of allFacs) {
  db.prepare('SELECT ComponentID FROM DataFacilityMounts WHERE ID=?').all(r.ID).map(x => x.ComponentID)
    .forEach(mid => db.prepare('SELECT ComponentID FROM DataMountWeapons WHERE ID=?').all(mid).forEach(x => usWpnDbIds.add(x.ComponentID)));
}

// US weapon designation prefixes
const US_WPN_PREFIXES = [
  'AIM-', 'AGM-', 'BGM-', 'RIM-', 'FIM-', 'UGM-', 'MGM-', 'MIM-',
  'GBU-', 'CBU-', 'BLU-', 'Mk', 'SLAM', 'HARM', 'NSM', 'LRASM',
  'THAAD', 'ATACMS', 'JDAM', 'JSOW', 'JASSM', 'Quickstrike',
  'ADM-', 'AGM-', 'Tomahawk', 'Harpoon', 'ESSM', 'Standard',
  'Stinger', 'Javelin', 'TOW', 'Hellfire', 'Hydra', 'Zuni',
  'APKWS', 'Excalibur', 'Copperhead', 'Griffin', 'GMLRS',
  'SDB', 'Walleye', 'Paveway', 'Skipper', 'Maverick',
  'SRAM', 'ALCM', 'CALCM', 'SLAM-ER', 'Phoenix', 'Sparrow',
  'Sidewinder', 'AMRAAM', 'GBI', 'Patriot', 'SM-',
  // Include gun calibers commonly used by US
  '5"/54', '5"/62', '127mm/54', '127mm/62',
  '76mm/62', '57mm/70', '25mm/75', '25mm/87',
  '20mm/85', '30mm', '20mm',
  // Torpedoes
  'Mk46', 'Mk48', 'Mk50', 'Mk54',
  // Mines
  'Mk36', 'Mk40', 'Mk41', 'Mk55', 'Mk56', 'Mk60', 'Mk62', 'Mk63', 'Mk64', 'Mk65',
  'Mk67', 'Quickstrike',
  // Rockets
  '70mm', '2.75"',
  // Nuclear
  'Trident', 'Minuteman', 'Peacekeeper',
  'W-',
];

function isUSWeapon(name) {
  return US_WPN_PREFIXES.some(p => name.startsWith(p));
}

// Get all matching weapons, group by family, take latest
const usWpns = {};
for (const wid of usWpnDbIds) {
  const w = db.prepare('SELECT * FROM DataWeapon WHERE ID=?').get(wid);
  if (!w || !isUSWeapon(w.Name)) continue;
  // Group by base designation
  const m = w.Name.match(/^([A-Z][A-Za-z\/"]+[-]?\d*[A-Z]*(?:\s*Mod\s*\d+)?)/);
  const family = m ? m[1] : w.Name.split(' ')[0];
  if (!usWpns[family]) usWpns[family] = w;
}

const wpnIndex = [], wpnDetails = {};
let wpnId = 0;
const sortedWpn = Object.entries(usWpns).sort((a, b) => a[0].localeCompare(b[0]));
for (const [family, row] of sortedWpn) {
  wpnId++;
  const displayName = cleanName(row.Name);
...
const usSnDbIds = new Set();
for (const r of allAC) {
  db.prepare('SELECT ComponentID FROM DataAircraftSensors WHERE ID=?').all(r.ID).forEach(x => usSnDbIds.add(x.ComponentID));
}
for (const r of allShips) {
  db.prepare('SELECT ComponentID FROM DataShipSensors WHERE ID=?').all(r.ID).forEach(x => usSnDbIds.add(x.ComponentID));
}
for (const r of allSubs) {
  db.prepare('SELECT ComponentID FROM DataSubmarineSensors WHERE ID=?').all(r.ID).forEach(x => usSnDbIds.add(x.ComponentID));
}
for (const r of allFacs) {
  db.prepare('SELECT ComponentID FROM DataFacilitySensors WHERE ID=?').all(r.ID).forEach(x => usSnDbIds.add(x.ComponentID));
}

// Filter to US-designated sensors (AN/ prefix is standard US designation)
const US_SN_PREFIXES = ['AN/', 'TADS', 'AAR-', 'AAS-', 'FLIR', 'GPS', 'Link', 'JTIDS', 'MIDS', 'TADIL', 'Mk'];

function isUSSensor(name) {
  return US_SN_PREFIXES.some(p => name.startsWith(p));
}

const usSensors = {};
for (const sid of usSnDbIds) {
  const s = db.prepare('SELECT * FROM DataSensor WHERE ID=?').get(sid);
  if (!s || !isUSSensor(s.Name)) continue;
  // Group by base name (strip version suffixes)
  const key = s.Name;
  if (!usSensors[key]) usSensors[key] = s;
}

const snIndex = [], snDetails = {};
let snId = 0;
const sortedSn = Object.entries(usSensors).sort((a, b) => a[0].localeCompare(b[0]));
